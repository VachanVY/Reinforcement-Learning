...
class Connect2Env(gym.Env):
    metadata = {"render_modes": ["human", "rgb_array", None], "render_fps": 30}
    
    def __init__(self, render_mode=None):
        super().__init__()
        self.render_mode = render_mode
        self.width = 4  # Four horizontal holes
        self.height = 1  # One row to allow connecting 2
        self.action_space = spaces.Discrete(self.width)  # Column selection
        self.observation_space = spaces.Box(low=-1, high=1, shape=(self.height, self.width), dtype=np.int8)
    ...
    def step(self, action):
        ...
        self.board[0, action] = self.current_player
        
        if self._check_win():
            return self.board.copy(), 10, True, False, {}
        
        if np.all(self.board != 0):
            return self.board.copy(), 0, True, False, {}  # Draw
    ...
    def render(self):
        ...
        if self.window is None:
            pygame.init()
            self.window = pygame.display.set_mode((400, 100))
            self.clock = pygame.time.Clock()
        ...
        for c in range(self.width):
            ...
            if self.board[0, c] == 1:
                ...
                color = (255, 255, 0)
            pygame.draw.circle(self.window, color, (c * cell_size + 50, 50), 40)
    ...
    def _check_win(self):
        for c in range(self.width - 1):
            if self.board[0, c] == self.current_player and self.board[0, c + 1] == self.current_player:
                return True
        return False
    
    @staticmethod
    def get_value_of_state(state:np.ndarray):
        for c in range(state.shape[1] - 1):
            if state[0, c] == state[0, c + 1] == 1:
                return 1 # Win!
            elif state[0, c] == state[0, c + 1] == -1:
                return -1 # Lose
        return 0 # Draw
...
if __name__ == "__main__":
    ...
    show_one_episode(env, env.action_space.sample, f"shortcut_maze_test.gif", title="sdsda")
    env.close()

    env = Connect2Env(render_mode="rgb_array")
    show_one_episode(env, env.action_space.sample, f"connect2_test.gif", title="sdsda")
    env.close()
