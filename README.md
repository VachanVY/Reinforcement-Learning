...
| 2017     | [Proximal Policy Optimization (PPO) -- Discrete Action Space](#)                         | **LunarLander-v3**: This environment is a classic rocket trajectory optimization problem. According to Pontryagin’s maximum principle, it is optimal to fire the engine at full throttle or turn it off                 | ![opaos](images/LunarLander-v3.gif)                 | ![Plot](images/LunarLander-v3_combined_plots.png) |
| 2017     | [Proximal Policy Optimization (PPO) -- Continuous Action Space](#)                         | **HalfCheetah-v5**: The goal is to apply torque to the joints to make the cheetah run forward (right) as fast as possible, with a positive reward based on the distance moved forward and a negative reward for moving backward. **Ant-v5**: The ant is a 3D quadruped robot consisting of a torso (free rotational body) with four legs attached to it, where each leg has two body parts. The goal is to coordinate the four legs to move in the forward (right) direction by applying torque to the eight hinges connecting the two body parts of each leg and the torso (nine body parts and eight hinges).                | ![opaos](images/PPO_Continuous_Actions_HalfCheetah-v5_GAE.gif) ![antgif](images/PPO_Continuous_Actions_Ant-v5_GAE.gif)                | ![Plot](images/PPO_tensorboard_log_HalfCheetah-v5_sum_rewards.png) ![Plot](images/PPO_HalfCheetah-v5_loss_charts_tensorboard.png) ![antplot](images/PPO_Cont_Actions_Ant-v5_GAE_training_curves.png)|
| 2018     | [Soft Actor-Critic (SAC)](#)                                    | **InvertedDoublePendulum-v5**: The cart can be pushed left or right, and the goal is to balance the second pole on top of the first pole, which is in turn on top of the cart, by applying continuous forces to the cart. | ![Plot](images/sac_adaptive_alpha_inverteddoublependulum-v5_.gif) | ![Plot](images/sac_rewards_adaptive_alpha_InvertedDoublePendulum-v5.png) |
...
### Advanced Deep Reinforcement Learning: Paper Implementations
